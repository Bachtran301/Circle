...
class TonCircle {
    ...
    async countdown(seconds) {
        for (let i = seconds; i >= 0; i--) {
            ...
            process.stdout.write(`===== Waiting ${i} seconds to continue looping =====`);
            ...
            for (const task of incompleteTasks) {
                ...
                if (startResult.success) {
                    ...
                    if (finalizeResult.success) {
                        this.log(`Successfully completed task ${task.data.title} (${type}) | reward: ${task.reward}`, 'success');
                    } else {
                        this.log(`Failed to complete task ${task.data.title} (${type}) | Manual action needed`, 'error');
                    }
                } else {
                    this.log(`Unable to start task ${task.data.title} (${type}): ${startResult.error}`, 'error');
                }
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        } else {
            this.log(`Unable to retrieve task list for ${type}: ${tasksResult.error}`, 'error');
        }
    }
    ...
    async processAdWatching(authorization, tg_id) {
        ...
        if (profileResult.success) {
            ...
            if (now < adWatchTime) {
                ...
                this.log(`Next ad can be watched in: ${timeUntilNextAd}`, 'custom');
            } else {
                const adResult = await this.getAd(tg_id);
                if (adResult.success) {
                    this.log('Starting to watch ad...', 'custom');
                    ...
                    if (watchResult.success) {
                        ...
                        this.log(`Error watching ad: ${watchResult.error}`, 'error');
                    }
                } else {
                    this.log(`Unable to fetch ad: ${adResult.error}`, 'error');
                }
            }
        } else {
            this.log(`Unable to fetch profile information: ${profileResult.error}`, 'error');
        }
    }
    ...
    async main() {
        while (true) {
                ...
                if (loginResult.success) {
                    ...
                    if (profileResult.success) {
                        ...
                        if (bonusResult.success) {
                            this.log(`Daily bonus claimed successfully!`, 'success');
                        } else {
                            this.log(`Failed to claim daily bonus: ${bonusResult.error}`, 'error');
                        }
                        ...
                        const updatedProfileResult = await this.getProfile(authorization);
                        if (updatedProfileResult.success) {
                            this.log(`Sparks available for spinning: ${updatedProfileResult.data.pointsBalance}`, 'custom');
                            if (updatedProfileResult.data.pointsBalance > 0) {
                                ...
                                if (spinResult.success) {
                                    ...
                                    this.log(`Failed to spin: ${spinResult.error}`, 'error');
                                }
                            }
                        } else {
                            this.log(`Unable to fetch updated profile information: ${updatedProfileResult.error}`, 'error');
                        }
            ...
            this.log('Processed all accounts. Pausing for 150 seconds before the next loop.', 'warning');
